...
public interface SocialControllerDocs {
  ...
  @GetMapping("/{id}")
  ResponseEntity<SocialResponse> selectSocialById(@PathVariable final Long id);
  ...
  @DeleteMapping("/{id}")
  ResponseEntity<String> cancelSocial(@PathVariable final Long id);

  @Operation(summary = "모임 상세 정보 조회", description = "모임의 상세 정보를 조회합니다.")
  @ApiResponse(responseCode = "200", description = "조회 성공")
  @GetMapping("/{id}/details")
  ResponseEntity<SocialDetailResponse> selectDetailBySocialId(@PathVariable final Long id);
  ...
  ResponseEntity<String> joinSocial(@PathVariable final Long id);
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "400",
        description = "취소 실패 - 이미 지났거나 참여하지 않은 모임. 혹은 모임 주최자의 취소 시도"),
    @ApiResponse(responseCode = "401", description = "취소 실패 - 로그인하지 않은 사용자"),
    @ApiResponse(responseCode = "404", description = "취소 실패 - 존재하지 않는 모임 아이디")
  })
  @DeleteMapping("/{id}/participants")
  ResponseEntity<String> leaveSocial(@PathVariable final Long id);
}
