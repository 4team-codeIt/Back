...
public class WebSecurityConfig {
	...
	private final List<Map<String, String>> excludeUrlsJwtfilter = List.of(
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(
						authorize -> {
							...
							excludeUrlsJwtfilter.forEach(entry -> {
								if ("ALL".equalsIgnoreCase(entry.get("method"))) {
									authorize.requestMatchers(entry.get("url")).permitAll();
								} else {
									authorize.requestMatchers(HttpMethod.valueOf(entry.get("method")), entry.get("url"))
											.permitAll();
								}
							});
				...
				.csrf(
						csrf -> {
							excludeUrlsJwtfilter.forEach(entry ->
									csrf.ignoringRequestMatchers(entry.get("url"))
							);
